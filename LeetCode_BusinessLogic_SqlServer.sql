...
select round(count(distinct p.player_id)*1.0/(select count(distinct Player_id) from Activity), 2) as fraction from Activity a
inner join temp p
...
/*Q10. Write a solution to find the daily active user count for a period of 30 days ending 2019-07-27 inclusively. A user was active on someday if they made at least one activity on that day.
...
/*Q11. Write a solution to find all sales that occurred in the first year each product was sold.
...
select product_id, year as first_year, quantity, price from products 
where rnk = 1;


/*Q12. Write a solution to:
...
with userRating as (
    select mr.user_id, u.name, count(mr.user_id) as cnt, dense_rank() over(order by count(mr.user_id) desc, name) as rnk1 from MovieRating mr
	inner join Users u on mr.user_id = u.user_id
	group by mr.user_id, u.name
),
movie as(
	select mr.movie_id, m.title, avg(cast(mr.rating as float)) as avgRating, dense_rank() over(order by avg(cast(mr.rating as float)) desc, title) as rnk2 from MovieRating mr
	inner join Movies m on mr.movie_id = m.movie_id
	where year(mr.created_at) = '2020' and month(mr.created_at) = '02'
	group by mr.movie_id,m.title
),
result as(
	select userRating.name as results from userRating
	where rnk1 = 1
	union all
